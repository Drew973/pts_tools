...
from PyQt5.QtWidgets import QStyledItemDelegate,QWidget,QHBoxLayout,QVBoxLayout,QAbstractItemView,QMenu
...
class fieldTable(QTableView):
    ...
    def __init__(self,parent=None):
        ...
        self.delegate1 = fieldBoxDelegate(self)
        self.setItemDelegateForColumn(0,self.delegate1)
        
        self.delegate2 = fieldBoxDelegate(self)
        self.setItemDelegateForColumn(1,self.delegate2)
        self.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.setSelectionBehavior(QAbstractItemView.SelectRows)
        #dataChanged
        
    def setBoxes(self,box1,box2):
        self.delegate1.setWidget(box1)
        self.delegate2.setWidget(box2)

        field1 = box1.currentField()
        field2 = box2.currentField()

        for i in range(0,self.model().rowCount()):
            self.model().setData(self.model().index(i,0),field1)
            self.model().setData(self.model().index(i,1),field2)
...
class fieldMapper(QWidget):
    
    def __init__(self,parent=None):
        super().__init__(parent)
        self.box1 = QgsFieldComboBox(self)
        self.box2 = QgsFieldComboBox(self)
        self.table = fieldTable(self)
        self.table.setBoxes(self.box1,self.box2)
        ...
        row.addWidget(self.box1)
        row.addWidget(self.box2)
    ...
    def add(self):
        self.table.model().appendRow([makeItem(self.box1.currentField()),makeItem(self.box2.currentField())])
...
def testFieldMapper():
    m = fieldMapper()
    m.box1.setLayer(layer1)
    m.box2.setLayer(layer2)
